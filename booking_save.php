...
//--------- Customer ---------------//
 ...
 $result->execute();
 for($i=0; $row = $result->fetch(); $i++){ $cus=$row['customer_name']; }



// --------- Save Booking --------//
$sql = "INSERT INTO booking (customer_name,vehicle_no,vehicle_id,type,type_id,emp_name,emp_id,date,booking_date,booking_time) VALUES (?,?,?,?,?,?,?,?,?,?)";
$ql = $db->prepare($sql);
$ql->execute(array($cus,$vehicle_no,$vehicle_id,$type,$type_id,$emp,$emp_id,$date,$booking_date,''));
